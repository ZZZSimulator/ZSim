...
class SweetScare(EnemySpecialState):
    ...
    def __init__(self, enemy_instance: "Enemy", manager_instance: "SpecialStateManager"):
        ...
        self.sugarburst_sparkless_max_trigger_origin_skill_tag: list[str] = ["1411_Assault_Aid_A", "1411_CoAttack_A"]
    ...
    def start(self):
        ...
        if YUZUHA_REPORT:
            ...
            print(f"【甜蜜惊吓】状态激活！本次状态预计于{sim_instance.tick + self.max_duration}tick结束")

    @property
    def sugarburst_sparkless_ready(self):
        if self.sugarburst_sparkless_update_tick == 0:
            return True
        if self.sim_instance.tick - self.sugarburst_sparkless_update_tick >= self.sugarburst_sparkless_cd:
            return True
        return False
    ...
    def update(self, update_signal: SSUS, **kwargs):
        ...
        if update_signal == SSUS.RECEIVE_HIT:
            ...
            print(f"【特殊状态：甜蜜惊吓 警告】接收到了与自己分组无关的信号{update_signal.value}，请检查逻辑以及数据库填写")
            return
    ...
    def attribute_changing(self, skill_node: "SkillNode"):
        ...
        if skill_node.skill_tag not in self.leagle_skill_tag:
            self.enemy.sim_instance.schedule_data.change_process_state()
            print(f"【特殊状态：甜蜜惊吓 警告】自检函数放行了一个不能被染色的技能：{skill_node.skill_tag}")
            return
    ...
    def flavor_match_update(self, skill_node: "SkillNode"):
        ...
        if YUZUHA_REPORT:
            self.enemy.sim_instance.schedule_data.change_process_state()
            print(f"【甜蜜惊吓】状态被技能{skill_node.skill_tag}染为 {ETM.get(skill_node.element_type)} 属性！这将在接下来的2400tick内改变所有【彩糖花火】的属性！")
    ...
    def __update_when_receive_hit(self, **kwargs):
        ...
        if single_hit.skill_tag in self.sugarburst_sparkless_max_trigger_origin_skill_tag:
            if single_hit.heavy_hit:
                ...
                if YUZUHA_REPORT:
                    ...
                    print(f"【甜蜜惊吓触发】由 {single_hit.skill_node.skill.skill_text} 触发了一次【彩糖花火·极】")
        ...
        if self.active:
            if not self.flavor_match:
                if int(single_hit.skill_tag.strip().split("_")[0]) == self.sim_instance.preload.preload_data.operating_now:
                    if single_hit.skill_node is None:
                        self.sim_instance.schedule_data.change_process_state()
                        print(f"【特殊状态：甜蜜惊吓警告】在试图更新染色状态时，检测到SingleHit并未关联SkillNode！染色失败！")
                        return
