...
class TestAPLDatabase:
    ...
    async def test_create_and_get_apl_config(self, setup_and_teardown):
        """测试创建和获取APL配置"""
        ...
        # 获取APL配置
        retrieved_config = await db.get_apl_config(config_id)
    ...
    async def test_update_apl_config(self, setup_and_teardown):
        """测试更新APL配置"""
        test_default_dir, test_custom_dir = setup_and_teardown
        db = await APLDatabase.creat()
        ...
        success = await db.update_apl_config(config_id, updated_data)
    ...
    async def test_delete_apl_config(self, setup_and_teardown):
        """测试删除APL配置"""
        test_default_dir, test_custom_dir = setup_and_teardown
        db = await APLDatabase.creat()
        ...
        # 删除配置
        success = await db.delete_apl_config(config_id)
        assert success is True
    ...
    async def test_get_apl_templates(self, setup_and_teardown):
        """测试获取APL模板"""
        test_default_dir, test_custom_dir = setup_and_teardown
        db = await APLDatabase.creat()
...
"""
        ...
        async with aiofiles.open(
            ...
            await f.write(default_toml_content)

        # 写入自定义模板文件
        async with aiofiles.open(
            os.path.join(test_custom_dir, "custom_template.toml"), "w", encoding="utf-8"
        ) as f:
            await f.write(custom_toml_content)

        # 获取模板
        templates = await db.get_apl_templates()
    ...
    async def test_get_apl_files(self, setup_and_teardown):
        """测试获取APL文件列表"""
        test_default_dir, test_custom_dir = setup_and_teardown
        db = await APLDatabase.creat()
        ...
        # 创建测试文件
        async with aiofiles.open(
            os.path.join(test_default_dir, "file1.toml"), "w", encoding="utf-8"
        ) as f:
            await f.write("# Test file 1")

        async with aiofiles.open(
            os.path.join(test_custom_dir, "file2.toml"), "w", encoding="utf-8"
        ) as f:
            await f.write("# Test file 2")

        # 获取文件列表
        files = await db.get_apl_files()
        assert len(files) == 2
    ...
    async def test_get_apl_file_content(self, setup_and_teardown):
        """测试获取APL文件内容"""
        test_default_dir, test_custom_dir = setup_and_teardown
        db = await APLDatabase.creat()
        ...
        content = await db.get_apl_file_content(file_id)
        assert content is not None
        assert content["content"] == test_content
        assert content["file_id"] == file_id

    async def test_create_apl_file(self, setup_and_teardown):
        """测试创建APL文件"""
        test_default_dir, test_custom_dir = setup_and_teardown
        db = await APLDatabase.creat()
        ...
        # 创建文件
        file_id = await db.create_apl_file(file_data)
    ...
    async def test_update_apl_file(self, setup_and_teardown):
        """测试更新APL文件"""
        test_default_dir, test_custom_dir = setup_and_teardown
        db = await APLDatabase.creat()
        ...
        success = await db.update_apl_file("custom_update_test.toml", new_content)
        assert success is True
    ...
    async def test_delete_apl_file(self, setup_and_teardown):
        """测试删除APL文件"""
        test_default_dir, test_custom_dir = setup_and_teardown
        db = await APLDatabase.creat()
        ...
        # 删除文件
        success = await db.delete_apl_file("custom_delete_test.toml")
        assert success is True
